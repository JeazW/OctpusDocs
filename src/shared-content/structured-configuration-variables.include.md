...
```
The namespace 'http://yamldoc.liuyan.wang' could not be mapped to the 'octopus' prefix, as another namespace 'http://yamldoc.liuyan.wang/xml' is already mapped to that prefix. XPath selectors using this prefix may not return the expected nodes. You can avoid this by ensuring all namespaces in your document have unique prefixes.
```
